...
from gflights.browser import BLOCKED_STOP_STATES
...
def test_live_google_flights_search_smoke_captures_task_scoped_artifacts(
    ...
    assert result.returncode in {0, 3, 4, 6}, result.stderr
    ...
    if result.returncode == 6:
        assert any(
            item["status"] == "tool_error" and item.get("stop_state") == "google_page_error"
            for item in payload
        )
